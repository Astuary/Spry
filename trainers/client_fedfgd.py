"""Client-side implementation of FedFgd"""
import os
import sys
...
from trainers.server_fedfgd import FedFgd
...
def agnews_train(net, train_data, test_data, seeds):
    ...
    for key, val in net.named_parameters():
        ...
        elif 'classifier' in key and peft_method == 'classifier':
            named_params[key] = val
    ...
    for e in range(epochs):
        ...
        for batch_index, batch in enumerate(train_data):
            ...
            v_params = []
            for seed, p in zip(seeds, params):
                if fixed_seed:
                    torch.manual_seed(seed)
                v_params.append(torch.randn_like(p).to(DEVICE))
            ...
            # Forward AD
            loss, jvp = fc.jvp(f, (tuple(params),), (tuple(v_params),))
    ...
    _, personalized_accuracy, _ = agnews_test(net, test_data)
...
def sst2_train(net, train_data, test_data, seeds):
    optimizer = SGD(net.parameters(), lr=learning_rate)
    net.train()
    ...
    average_time_per_epoch = sum(time_per_epoch) / len(time_per_epoch)
    average_time_per_iteration_per_epoch = sum(time_per_iteration_per_epoch) / len(time_per_iteration_per_epoch)
    _, personalized_accuracy, _ = sst2_test(net, test_data)
...
def sst2_generalized_test(net, train_data, test_data):
    before_loss, before_accuracy, sample_count = sst2_test(net, test_data)
    after_loss, after_accuracy = before_loss, before_accuracy
    # print("Accuracy: ", correct, "/", total, " = ", accuracy)
    return before_loss, before_accuracy, after_loss, after_accuracy, sample_count

def question_answerer_train(net, train_dataloader, test_dataloader, test_dataset, test_examples, seeds):
    if client_opt == 'sgd':
        optimizer = SGD(net.parameters(), lr=learning_rate)
...
def text_classifier_train(net, train_data, test_data, seeds):
    if client_opt == 'sgd':
        optimizer = SGD(net.parameters(), lr=learning_rate)
    elif client_opt == 'adamw':
        optimizer = AdamW(net.parameters(), lr=learning_rate)
...
class QuestionAnswererClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        (loss, 
         ...
         personalized_f1, 
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch) = question_answerer_train(self.net, train_dataloader, test_dataloader, test_dataset, test_examples, config['seeds'])
        torch.cuda.empty_cache()
...
class TextClassifierClient(fl.client.NumPyClient):
    ...
    def get_parameters(self, config):
        ...
        for key, val in self.net.named_parameters():
            if val.requires_grad and peft_method == 'lora':
                trainable_keys.append(key)
            if val.requires_grad and peft_method == 'ia3':
                trainable_keys.append(key)
            elif 'bias' in key and peft_method == 'bitfit':
                trainable_keys.append(key)
            elif 'classifier' in key and peft_method == 'classifier':
    ...
    def fit(self, parameters, config):
        ...
        (loss, count, personalized_accuracy, 
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch) = text_classifier_train(self.net, train_data, test_data, config['seeds'])
        torch.cuda.empty_cache()
...
def client_fn(cid: str):# -> fl.client.Client:
    ...
    if dataset in ['agnews', 'yahoo', 'sst2', 'mnli', 'snli', 'qnli', 'cola', 'yelp', 'qqp']:
        ...
        if peft_method == 'lora':
            ...
            net = get_peft_model(net, lora_config).to(DEVICE)
        elif peft_method == 'ia3':
            ia3_config = IA3Config(
                task_type=TaskType.QUESTION_ANS, 
            )
            net = get_peft_model(net, ia3_config).to(DEVICE)
