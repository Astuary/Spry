"""Client-side implementation of FedYogi"""
import os
import sys
...
from trainers.server_fedyogisplit import FedYogi
...
def question_answerer_train(net, train_dataloader, test_dataloader, test_dataset, test_examples, trainable_keys):
    if client_opt == 'sgd':
    ...
    for key, val in net.named_parameters():
        if val.requires_grad and peft_method == 'lora':
            if index in split_trainable_keys:
    ...
    # _, personalized_accuracy, _ = squad_test(net, test_data)
...
def question_answerer_generalized_test(net, train_dataloader, test_dataloader, test_dataset, test_examples):
    ...
    # before_loss, before_accuracy, sample_count = squad_test(net, test_data)
...
def text_classifier_train(net, train_data, test_data, split_trainable_keys):
    net.train()
    if client_opt == 'sgd':
        optimizer = SGD(net.parameters(), lr=learning_rate)
    elif client_opt == 'adamw':
...
class QuestionAnswererClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        (loss, 
         ...
         personalized_f1, 
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch) = question_answerer_train(
             self.net, train_dataloader, test_dataloader, test_dataset, test_examples,
...
class TextClassifierClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        (loss, count, personalized_accuracy, 
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch) = text_classifier_train(self.net, 
