...
class FedAvg(fl.server.strategy.FedYogi):
    ...
    def __init__(
        ...
        self.var_threshold = self.hyperparameters['var_threshold']
        try:
    ...
    def configure_fit(
        ...
        for idx, i in enumerate(client_proxy_and_fitins):
            ...
            i[1].config['current_gradients'] = self.current_gradients

        # print('In configure_fit', self.current_gradients, flush=True)
        # for idx, i in enumerate(client_proxy_and_fitins):
        #     print('In configure_fit', len(i[1].config['current_gradients']), flush=True)
    ...
    def aggregate_fit(
        ...
        losses = [r.metrics["loss"] * r.num_examples for _, r in results if r.metrics["var"] < self.var_threshold]
        
        examples = [r.num_examples for _, r in results if r.metrics["var"] < self.var_threshold]
        average_time_per_epoch = [r.metrics["average_time_per_epoch"] for _, r in results if r.metrics["var"] < self.var_threshold]
        average_time_per_iteration_per_epoch = [r.metrics["average_time_per_iteration_per_epoch"] for _, r in results if r.metrics["var"] < self.var_threshold]
        ...
        if 'squad' in self.dataset_name:
            personalized_exact_matches = [r.metrics["personalized_exact_match"] * r.num_examples for _, r in results if r.metrics["var"] < self.var_threshold]
            personalized_f1 = [r.metrics["personalized_f1"] * r.num_examples for _, r in results if r.metrics["var"] < self.var_threshold]
            ...
            personalized_accuracies = [r.metrics["personalized_accuracy"] * r.num_examples for _, r in results if r.metrics["var"] < self.var_threshold]
        ...
        all_losses = [r.metrics["loss"] for _, r in results if r.metrics["var"] < self.var_threshold]
        
        if 'squad' in self.dataset_name:
            all_personalized_exact_matches = [r.metrics["personalized_exact_match"] for _, r in results if r.metrics["var"] < self.var_threshold]
            all_personalized_f1 = [r.metrics["personalized_f1"] for _, r in results if r.metrics["var"] < self.var_threshold]
            ...
            all_personalized_accuracies = [r.metrics["personalized_accuracy"] for _, r in results if r.metrics["var"] < self.var_threshold]
        ...
        if len(self.previous_weights) > 0:
            self.current_gradients = [None] * len(aggregated_weights)
            for index, current_w in enumerate(aggregated_weights):
                self.current_gradients[index] = current_w - self.previous_weights[index]
                self.previous_weights[index] = current_w
            # print('In aggregate_fit current gradients', len(self.current_gradients), flush=True)
        else:
            self.previous_weights = []
            for index, current_w in enumerate(aggregated_weights):
                self.previous_weights.append(current_w)
            # print('In aggregate_fit previous_weights', len(self.previous_weights), flush=True)
