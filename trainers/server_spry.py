...
from flwr.common import (
    Parameters, Scalar, MetricsAggregationFn, 
...
class FedFgd(fl.server.strategy.FedYogi):
    ...
    def __init__(
        ...
        dataset: str = 'agnews',
        client_algorithm: str = 'FedFgdSplit',
        ...
        self.classifier_share = self.hyperparameters['classifier_share']
        self.per_client_layer_count = self.hyperparameters['per_client_layer_count']
        ...
        for key, val in self.net.named_parameters():
            if val.requires_grad and self.peft_method == 'lora':
                ...
                self.trainable_keys.append(key)
                # print('key: ', key, val.shape, flush=True)
                
        print('self.trainable_keys', len(self.trainable_keys), flush=True)
        
        self.trainable_keys_mapping = {}
        if self.classifier_share and self.model_name in ['bert-base-uncased', 'albert-large-v2', 'bert-large-uncased']:
            self.classifier_layer_count = 2
            for client_idx, key_idx in enumerate(range(0, len(self.trainable_keys) - self.classifier_layer_count, 2)):
                rollover_idx = client_idx % self.num_clients
                if rollover_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[rollover_idx].extend([key_idx, key_idx+1])
                    # self.trainable_keys_mapping[rollover_idx] = self.trainable_keys_mapping[rollover_idx ].extend([self.trainable_keys[key_idx], self.trainable_keys[key_idx+1]])
                else:
                    self.trainable_keys_mapping[rollover_idx] = [key_idx, key_idx+1]
                    # self.trainable_keys_mapping[rollover_idx] = [self.trainable_keys[key_idx], self.trainable_keys[key_idx+1]]

            for client_idx in range(self.num_clients):
                if client_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[client_idx].extend([len(self.trainable_keys) - 2, len(self.trainable_keys) - 1])
                else:
                    self.trainable_keys_mapping[client_idx] = [len(self.trainable_keys) - 2, len(self.trainable_keys) - 1]

        elif self.classifier_share and self.model_name in ['distilbert-base-uncased', 'roberta-large']:
            self.classifier_layer_count = 4
            for client_idx, key_idx in enumerate(range(0, len(self.trainable_keys) - self.classifier_layer_count, 2)):
                rollover_idx = client_idx % self.num_clients
                if rollover_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[rollover_idx].extend([key_idx, key_idx+1])
            ...
            for client_idx in range(self.num_clients):
                if client_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[client_idx].extend([len(self.trainable_keys) - 4, len(self.trainable_keys) - 3, len(self.trainable_keys) - 2, len(self.trainable_keys) - 1])
                else:
                    self.trainable_keys_mapping[client_idx] = [len(self.trainable_keys) - 4, len(self.trainable_keys) - 3, len(self.trainable_keys) - 2, len(self.trainable_keys) - 1]

        elif self.classifier_share and 'llama' in self.model_name:
            self.classifier_layer_count = 1
            for client_idx, key_idx in enumerate(range(0, len(self.trainable_keys) - self.classifier_layer_count, 2)):
                rollover_idx = client_idx % self.num_clients
                if rollover_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[rollover_idx].extend([key_idx, key_idx+1])
            ...
            for client_idx in range(self.num_clients):
                if client_idx in self.trainable_keys_mapping.keys():
                    self.trainable_keys_mapping[client_idx].extend([len(self.trainable_keys) - 1])
                else:
                    self.trainable_keys_mapping[client_idx] = [len(self.trainable_keys) - 1]
        
        else:
            self.classifier_layer_count = None
            if self.peft_method == 'lora':
                for client_idx, key_idx in enumerate(range(0, len(self.trainable_keys), self.per_client_layer_count)):
                    rollover_idx = client_idx % self.num_clients
                    if rollover_idx in self.trainable_keys_mapping.keys():
                        for sub_idx in range(self.per_client_layer_count):
                            if key_idx + sub_idx < len(self.trainable_keys): 
                                self.trainable_keys_mapping[rollover_idx].append(key_idx + sub_idx)
                            else:
                                break
                        # self.trainable_keys_mapping[rollover_idx] = self.trainable_keys_mapping[rollover_idx ].extend([self.trainable_keys[key_idx], self.trainable_keys[key_idx+1]])
                    else:
                        self.trainable_keys_mapping[rollover_idx] = [key_idx]
                        for sub_idx in range(1, self.per_client_layer_count):
                            if key_idx + sub_idx < len(self.trainable_keys):
                                self.trainable_keys_mapping[rollover_idx].append(key_idx + sub_idx)
                            else:
                                break
            elif self.peft_method == 'bitfit' or self.peft_method == 'ia3':
                for client_idx, key_idx in enumerate(range(0, len(self.trainable_keys))):
                    rollover_idx = client_idx % self.num_clients
                    if rollover_idx in self.trainable_keys_mapping.keys():
                        self.trainable_keys_mapping[rollover_idx].extend([key_idx])
                        # self.trainable_keys_mapping[rollover_idx] = self.trainable_keys_mapping[rollover_idx ].extend([self.trainable_keys[key_idx], self.trainable_keys[key_idx+1]])
                    else:
                        self.trainable_keys_mapping[rollover_idx] = [key_idx]
                        # self.trainable_keys_mapping[rollover_idx] = [self.trainable_keys[key_idx], self.trainable_keys[key_idx+1]]
    ...
    def configure_fit(
        ...
        # valid_seeds = [self.current_seed for _ in range(len(self.trainable_keys))]
        
        # print("Valid Seeds: ", valid_seeds)
        for idx, i in enumerate(client_proxy_and_fitins):
            i[1].config['rnd'] = server_round
            i[1].config['seed'] = self.current_seed
            i[1].config['trainable_keys'] = self.trainable_keys_mapping[idx]
            # print('client trainable_keys_mapping', idx, ': ', self.trainable_keys_mapping[idx], flush=True)
    ...
    def aggregate_fit(
        ...
        # aggregated_weights = [None] * len(self.trainable_keys)
        # for _, r in results:
        #     for index, param_index in enumerate(r.metrics["trainable_keys"]):
        #         aggregated_weights[param_index] = parameters_to_ndarrays(r.parameters)[index]
        # parameters_aggregated = ndarrays_to_parameters(aggregated_weights)
        # aggregated_parameters_tuple = (parameters_aggregated, {})
        
        results[0][1].metrics['classifier_share'] = self.classifier_share
        results[0][1].metrics['classifier_layer_count'] = self.classifier_layer_count
