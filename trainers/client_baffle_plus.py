                ...
                if finite_difference_format == "forward":
                    ...
                    for i in range(K):
                        ...
                        for name, param in net.named_parameters():
                            if param.requires_grad:
                                delta[name] = sigma * torch.randn(param.size()).to(DEVICE)
                                param += delta[name]
                        ...
                        for name, param in net.named_parameters():
                            if param.requires_grad:
                                if param.grad != None:
                                    param.grad += (difference / sigma) * (delta[name] / sigma)
                                else:
                                    param.grad = (difference / sigma) * (delta[name] / sigma)
                                param -= delta[name]
                    ...
                    for i in range(K):
                        ...
                        for name, param in net.named_parameters():
                            if param.requires_grad:
                                if param.grad != None:
                                    param.grad += (difference / 2 * sigma) * (delta[name] / sigma)
                                else:
                                    param.grad = (difference / sigma) * (delta[name] / sigma)
                                param += delta[name]
