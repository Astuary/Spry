"""Client-side implementation of FwdLLM"""
import os
import sys
...
import dataloaders.boolq.boolq_dataloader as boolq_dataloader
...
from trainers.server_fwdllm import FedAvg
...
def functional_get_loss(
    ...
    print('x.keys()', x.keys(), flush=True)
...
def question_answerer_train(net, train_data, test_dataloader, test_dataset, test_examples, previous_gradients):
# def question_answerer_train(net, train_data, test_data):
    ...
    for key, val in net.named_parameters():
        if 'bias' in key and peft_method == 'bitfit':
            named_params[key] = val
        elif val.requires_grad and peft_method == 'lora':
            named_params[key] = val
    ...
    v_num = (len(train_data) + 1) * batch_size
    
    v_buffer = {}
    index = 0    
    for k, v in net.named_parameters():
        if len(previous_gradients) > 0 and v.requires_grad:
            shape = v.shape
            
            candidate_v = torch.randn((v_num * 10, *shape), device="cpu")
            target_grad = torch.from_numpy(previous_gradients[index])
            
            target_grad = torch.flatten(target_grad)
            candidate_v = torch.flatten(candidate_v, start_dim=1)

            cos_sim = calculate_cos_sim(candidate_v, target_grad, 'cpu')
        ...
        for batch_index, batch in enumerate(train_data):
            ...
            for perturbation_index in range(perturbation_count):
                with torch.no_grad():
                    if v_buffer != {}:
                        ...
                        for i, p in enumerate(params):
                            if p.requires_grad == True:
                                v_params.append(v_buffer[j][batch_index].to(DEVICE))
                                j += 1
                        v_params = tuple(v_params)
                        
                    else:
                        v_params = tuple([torch.randn_like(p) for p in params if p.requires_grad])
                    ...
                    # Forward AD
                    loss, jvp = calculate_jvp(f, tuple(params), v_params)
                    ...
                    for p in params:
                        if p.requires_grad:
                            if p.grad == None:
                                p.grad = v_params[index] * jvp
                            else:
                                p.grad.data += v_params[index] * jvp
                            index += 1
                
            for grad_index, p in enumerate(params):
            # for grad_index, p in enumerate(net.parameters()):
                if p.requires_grad:
                    p.grad.data /= perturbation_count
                
                    if grad_index == layer_id_for_check:
                        grad_for_var_check_list.append(p.grad.data)
...
def text_classifier_train(net, train_data, test_data, previous_gradients):
    net.train()
    if client_opt == 'sgd':
        optimizer = SGD(net.parameters(), lr=learning_rate)
    elif client_opt == 'adamw':
    ...
    for e in range(epochs):
        ...
        for batch_index, batch in enumerate(train_data):
                    ...
                    if v_buffer != {}:
                        ...
                        for i, p in enumerate(params):
                            if p.requires_grad == True:
                                ...
                                j += 1
                            # else:
                            #     v_params.append(torch.zeros_like(p).to(DEVICE))
                        v_params = tuple(v_params)
            ...
            # for grad_index, p in enumerate(net.parameters()):
                if p.requires_grad:
                    ...
                    if grad_index == layer_id_for_check:
                        grad_for_var_check_list.append(p.grad.data)
                # if p.requires_grad:
                #     print(grad_index, layer_id_for_check)
            ...
            # for p in net.parameters():
            #     if p.requires_grad:
            #         print('net p: ', p[0][0], flush=True)
            #         print('net p.grad: ', p.grad[0][0], flush=True)
            #         break
            
            # for p in params:
            #     if p.requires_grad:
            #         print('params p: ', p[0][0], flush=True)
            #         print('params p.grad: ', p.grad[0][0], flush=True)
            #         break
            # print('-----')
    ...
    var = 0.0 # calculate_var(grad_for_var_check_list)
...
class QuestionAnswererClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        (loss, 
         ...
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch,
         var) = question_answerer_train(self.net, train_dataloader, test_dataloader, test_dataset, test_examples, config['current_gradients'])
        torch.cuda.empty_cache()
...
class TextClassifierClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        (loss, count, personalized_accuracy, 
         average_time_per_epoch, 
         average_time_per_iteration_per_epoch,
         var) = text_classifier_train(self.net, train_data, test_data, config['current_gradients'])
        torch.cuda.empty_cache()
...
def client_fn(cid: str):# -> fl.client.Client:
    ...
    if dataset == 'agnews': 
        ...
        net.config.label2id = {'True': 0, 'False': 1,}
        net.config.id2label = {0: 'True', 1: 'False',}
        return TextClassifierClient(cid, net)
...
if __name__  == '__main__':
    ...
    hyperparameters['perturbation_scale'] = perturbation_scale
    var_threshold = float(args.var_threshold)
    hyperparameters['var_threshold'] = var_threshold
    ...
    if dataset == 'agnews':
        ...
        train_dataloader_dict, test_dataloader_dict = boolq_dataloader.get_federated_datasets(
            dirichlet_parameter=dirichlet_distribution,
            num_clients=total_clients, 
            train_client_batch_size=batch_size,
            max_seq_len=max_seq_len,
    ...
    if 'distilbert' in model_name:
        layer_id_for_check = 130
    elif 'roberta' in model_name:
        layer_id_for_check = 489
    elif 'albert' in model_name:
        layer_id_for_check = 33
    elif 'bert' in model_name:
        layer_id_for_check = 249
    elif 'llama' in model_name:
        layer_id_for_check = 0
    elif 'opt' in model_name:
        layer_id_for_check = 0
        
    hyperparameters['layer_id_for_check'] = layer_id_for_check
