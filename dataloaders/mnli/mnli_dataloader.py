"""Load MNLI data (training and eval)"""
...
def load_mnli_federated(
    dirichlet_parameter: float = 0.1,
    num_clients: int = 500,
    max_seq_len: int = 64,
    split: float = 0.8,
    random_seed: int = 0,
    cache_dir: str = './dataset_cache/mnli',
    tokenizer_name: str = 'bert-base-uncased',
    save_dir: str = './dataset_cache/mnli/manual_save_500_0_1',
    ...
    raw_datasets = load_dataset('glue', 'mnli', cache_dir=cache_dir)
    ...
    inputs_1 = np.hstack(
        ...
        # np.array(raw_datasets['validation_mismatched']['premise'] + raw_datasets['validation_mismatched']['hypothesis']), 
        np.array(raw_datasets['validation_matched']['premise']),
        ))
    inputs_2 = np.hstack(
        (np.array(raw_datasets['train']['hypothesis']), 
        # np.array(raw_datasets['validation_mismatched']['premise'] + raw_datasets['validation_mismatched']['hypothesis']), 
        np.array(raw_datasets['validation_matched']['hypothesis']),
        ))
    labels = np.hstack(
        (np.array(raw_datasets['train']['label']), 
        # np.array(raw_datasets['validation_mismatched']['label']),
        np.array(raw_datasets['validation_matched']['label']),
        ))
...
def get_federated_datasets(
    ...
    if not os.path.exists(save_dir):
        ...
        mnli_train_all_data, mnli_test_all_data = load_mnli_federated(
        ...
        mnli_train_all_data, mnli_test_all_data = get_mnli_federated(
            num_clients = num_clients,
            save_dir = save_dir,
        )
    
    mnli_train_dataloader_dict = {}
    mnli_test_dataloader_dict = {}
    
    for client in range(num_clients):
        
        mnli_train_dataloader_dict[str(client)] = DataLoader(
            mnli_train_all_data[str(client)],
            batch_size=train_client_batch_size,
            num_workers=2
        )
        
        mnli_test_dataloader_dict[str(client)] = DataLoader(
            mnli_test_all_data[str(client)],
            batch_size=test_client_batch_size,
            num_workers=2
        )
    
    return mnli_train_dataloader_dict, mnli_test_dataloader_dict
