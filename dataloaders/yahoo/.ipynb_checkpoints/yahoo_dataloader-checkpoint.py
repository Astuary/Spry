...
def merge_columns(example):
    merged_value = str(example['question_title']) + ' ' + \
        str(example['question_content']) + ' ' + str(example['best_answer'])
    
    # Update the example with the merged value
    example['question_answer'] = merged_value
    return example

def load_yahoo_federated(
    dirichlet_parameter: float = 0.1,
    num_clients: int = 500,
    max_seq_len: int = 128,
    split: float = 0.8,
    cache_dir: str = './dataset_cache/yahoo-bert-base-uncased',
    tokenizer_name: str = 'bert-base-uncased',
    save_dir: str = './dataset_cache/yahoo-bert-base-uncased/manual_save_500_0_1',
    ...
    """
    raw_datasets = load_dataset("yahoo_answers_topics", cache_dir=cache_dir)
    raw_datasets = raw_datasets.shuffle(seed=42)
    raw_datasets = raw_datasets.map(merge_columns)
    
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)

    inputs = np.hstack(
        (np.array(raw_datasets['train']['question_answer']), np.array(raw_datasets['test']['question_answer'])),
        )
    labels = np.hstack(
        (np.array(raw_datasets['train']['topic']), np.array(raw_datasets['test']['topic'])),
        )
    ...
    for i in range(num_clients):
        print('Preparing Dataset of Client #' + str(i))
        client_name = str(i)
...
def get_federated_datasets(
    ...
    max_seq_len: int = 128,
    cache_dir: str = './dataset_cache/yahoo',
    tokenizer_name: str = 'bert-base-uncased',
):
    cache_dir = cache_dir + '-' + tokenizer_name
    save_dir = cache_dir + '/manual_save_' + str(num_clients) + '_' + str(dirichlet_parameter).replace('.', '_')
    
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
        yahoo_train_all_data, yahoo_test_all_data = load_yahoo_federated(
        ...
        yahoo_train_all_data, yahoo_test_all_data = get_yahoo_federated(
            num_clients = num_clients,
            save_dir = save_dir,
        )
    
    yahoo_train_dataloader_dict = {}
    yahoo_test_dataloader_dict = {}
    
    for client in range(num_clients):
        
        yahoo_train_dataloader_dict[str(client)] = DataLoader(
            yahoo_train_all_data[str(client)],
            batch_size=train_client_batch_size,
            num_workers=2
        )
        
        yahoo_test_dataloader_dict[str(client)] = DataLoader(
            yahoo_test_all_data[str(client)],
            batch_size=test_client_batch_size,
            num_workers=2
        )
    
    return yahoo_train_dataloader_dict, yahoo_test_dataloader_dict
