"""Load Yahoo data (training and eval)"""
...
def load_yahoo_federated(
    ...
    save_dir: str = './dataset_cache/yahoo/manual_save_500_0_1',
    ...
    label_list = raw_datasets["train"].features["topic"].names
    ...
    inputs = np.hstack(
        (np.array(raw_datasets['train']['question_title']), 
         np.array(raw_datasets['test']['question_title'])),
        )
    labels = np.hstack(
        (np.array(raw_datasets['train']['topic']), 
         np.array(raw_datasets['test']['topic'])),
        )
