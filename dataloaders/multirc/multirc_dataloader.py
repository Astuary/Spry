"""Load multirc data (training and eval)"""
...
def load_multirc_federated(
    ...
    save_dir: str = './dataset_cache/multirc/manual_save_500_0_1',
    ...
    raw_datasets = load_dataset('super_glue', 'multirc', cache_dir=cache_dir)
    ...
    def merge_columns(example):
        merged = example["question"] + " " + example["answer"]
        return {"question_answer": merged}
    raw_datasets = raw_datasets.map(merge_columns)

    inputs_1 = np.hstack(
        (np.array(raw_datasets['train']['paragraph']), 
        np.array(raw_datasets['validation']['paragraph']), 
        np.array(raw_datasets['test']['paragraph']),
        ))
    inputs_2 = np.hstack(
        (np.array(raw_datasets['train']['question_answer']), 
        np.array(raw_datasets['validation']['question_answer']), 
        np.array(raw_datasets['test']['question_answer']),
        ))
    labels = np.hstack(
    ...
    example_count = examples_label_0 + examples_label_1
...
def get_federated_datasets(
    ...
    # cache_dir = cache_dir + '-' + tokenizer_name
    save_dir = cache_dir + '-' + tokenizer_name.split('/')[-1] + '/manual_save_' + str(num_clients) + '_' + str(dirichlet_parameter).replace('.', '_')
    
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
        multirc_train_all_data, multirc_test_all_data = load_multirc_federated(
        ...
        multirc_train_all_data, multirc_test_all_data = get_multirc_federated(
            num_clients = num_clients,
            save_dir = save_dir,
        )
    
    multirc_train_dataloader_dict = {}
    multirc_test_dataloader_dict = {}
    
    for client in range(num_clients):
        
        multirc_train_dataloader_dict[str(client)] = DataLoader(
            multirc_train_all_data[str(client)],
            batch_size=train_client_batch_size,
            num_workers=2
        )
        
        multirc_test_dataloader_dict[str(client)] = DataLoader(
            multirc_test_all_data[str(client)],
            batch_size=test_client_batch_size,
            num_workers=2
        )
    
    return multirc_train_dataloader_dict, multirc_test_dataloader_dict
