...
def functional_fwd_pass(
    params: ValuesView,
    buffers: Dict[str, Tensor],
    names: KeysView,
    model: torch.nn.Module,
    batch_dict: torch.Tensor,
) -> torch.Tensor:
    """Functional cross-entropy loss. Given a pytorch model it computes the cross-entropy loss
    in a functional way.
    ...
    Args:
        ...
        names: Names of the parameters.
        model: A pytorch model.
        batch_dict (torch.Tensor): Batched input and output for the PyTorch model.

    Returns:
        torch.Tensor: Cross-entropy loss.
    """
    y = fc.functional_call(model, ({k: v for k, v in zip(names, params)}, buffers), (), batch_dict)
    return y.loss